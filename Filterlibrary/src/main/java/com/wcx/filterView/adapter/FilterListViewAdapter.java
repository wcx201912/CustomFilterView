package com.wcx.filterView.adapter;
...
import com.wcx.filterView.view.SkuFlowLayout;
...
public class FilterListViewAdapter extends BaseAdapter {
    ...
    private OnCheckedTextChangeListener onCheckedTextChangeListener;

    public FilterListViewAdapter(Context context, List<FiltrateBean> dictList) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        final ViewHolder vh;
        if (convertView == null) {
            convertView = View.inflate(context, R.layout.item_filter_list, null);
            vh = new ViewHolder();
    ...
    private void setFlowLayoutData(final List<FiltrateBean.Children> childrenList, final SkuFlowLayout flowLayout, final int position) {
        ...
        for (int x = 0; x < childrenList.size(); x++) {
            final CheckBox checkBox = (CheckBox) View.inflate(context, R.layout.item_checkbox, null);
        ...
        for (int y = 0; y < flowLayout.getChildCount(); y++) {
            ...
            if (finalX == y) {
                ...
                if (propBeenList.get(y).isSelected()) {
                    radio.setChecked(false);
                    propBeenList.get(y).setSelected(false);
                    if (onCheckedTextChangeListener != null) {
                        onCheckedTextChangeListener.onTextChange(propBeenList.get(y).getValue(), false);
                    }
                } else {
                    radio.setChecked(true);
                    propBeenList.get(y).setSelected(true);
                    if (onCheckedTextChangeListener != null) {
                        onCheckedTextChangeListener.onTextChange(propBeenList.get(y).getValue(), true);
                    }
                }
            }
        }
    ...
    public interface OnCheckedTextChangeListener {
        void onTextChange(String text, boolean checked);
    }

    public void setCheckedTextChangeListener(OnCheckedTextChangeListener onCheckedTextChangeListener) {
        this.onCheckedTextChangeListener = onCheckedTextChangeListener;
    }
}
